...
- 采集节奏（`discovery_manifest.json > cadence_plan`）是对"首轮起点"的运维收窄，不改变上面这条原则：脚本按命中率把长尾面降到隔日/每周，`due=true` 的面必须全跑；非 due 面仍可被 AI 因外部信号唤醒，但必须在 attempt 里写非空 `wakeup_reason`（finalize 强制，缺则失败）。cadence 只决定"今天探不探"，不参与正文取舍与排序。审计面由 manifest 全量留痕承担，不因降频变窄。
- 校验类函数的契约必须由签名强制，**禁止用「省略某参数就退回弱校验」的可选参数**：那等于把契约强度交给调用点，新增调用误用只是时间问题。需要表达「暂无此信息」时用显式必传的 `None`（调用方必须写出来），不要用默认值。同理，同一个事实不得由两个参数分别传入（如 due 名单与 cadence plan），派生关系要在函数内部完成。
- 当被校验的对象是本流程某个纯函数的输出（如 `cadence_plan` 之于 `compute_cadence`）时，**判据必须是重算比对，不是逐字段规则**：逐字段规则只是对「是否等于该函数输出」这个等价判断的近似，近似必然留下绕过路径（PR #5 为此返工五轮）。
