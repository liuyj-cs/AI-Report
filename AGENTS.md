...
- 校验类函数的契约必须由签名强制，**禁止用「省略某参数就退回弱校验」的可选参数**：那等于把契约强度交给调用点，新增调用误用只是时间问题。需要表达「暂无此信息」时用显式必传的 `None`（调用方必须写出来），不要用默认值。同理，同一个事实不得由两个参数分别传入，派生关系要在函数内部完成。
- 当被校验的对象是本流程某个纯函数的输出时，**判据必须是重算比对，不是逐字段规则**：逐字段规则只是对「是否等于该函数输出」这个等价判断的近似，近似必然留下绕过路径（PR #5 为此返工五轮，最终把那套调度整个删掉）。
- 加机制前先算这笔账：它覆盖多少比例的实际场景、省下什么、以及要多少行代码和多少次 review 才能稳住。收益被前提条件锁死在低位的机制（PR #5 的采集调度：74 个面里 48 个恒每日，可降频的只有 24 个，实测省 21%）不值得引入——宁可不做，也不要留一套复杂且不一定对的逻辑。
- 任何「这里不该再有 X」的断言（删除、退役、改名、撤销），必须落成一条**覆盖全仓的可执行检查**（一次 `grep -rn --exclude-dir=.git`），再把剩余命中逐条判定为「历史附录」或「故意的禁止测试」；**禁止凭记忆列举要改的位置**。检查范围必须含 `docs/`——spec-driven 的 spec/plan 是契约不是叙述，实现与 active 需求互斥就是 bug（PR #5 删掉 cadence 后代码全清、spec 里却还留着 `cadence: daily` pin 的要求，与新增的禁止测试直接矛盾）。
- 写进 PR、commit、验收结论里的任何数字（测试数、条目数、耗时），必须**在最终提交的那棵树上重新取一次**，不得复述早前某次运行的结果。这跟守门「只认 attempts 留痕、不认自报字段」是同一条纪律，对自己的完成报告同样成立。
...
- 重大事件与独立专题分别判断：事件继续补证和追踪；专题仅在解决具体选择问题、比晨报增加实质分析且证据就绪时产出。AI 复核后通过日报 `deep_dive_refs` 显式选择投递，不从重大事件或磁盘文件自动推断；详见 skill 的 `workflows/deep-dive-research.md`。每天晨报仍全文同步到已授权的“AI信息晨报”钉钉目录。
