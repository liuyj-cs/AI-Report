...
摩擦点（2026-07-27）：
1. **标注取证方式是最大教训**：T6 按 URL 语义标注 65 条链，测试只能守恒"标了没有"、守不住"标得对不对"。两轮 pre-close review 实抓后共 20 个面从 feed 改判 static，其中 MiniMax news 与 Claude Code release-notes（core_source）若不改会直接重开第五批修过的「召回静默漏采」。根因归类：**测试盲区**。后续新增信源必须实抓验证再标 surface_kind。
2. **守门建立在自我声明上等于没有守门**：首版链内穷尽判据读 AI 自填的 `final_layer_index`，实测只抓 Layer-0 自报 2 即可绕过全部保护。根因归类：**隐式假设**（默认 AI 会诚实填写元数据）。改为只认 `attempts[]` 留痕。
3. **fail-open 方向错误出现两次**：`due_discovery_names` 返回空列表被当成有效空基准（未来日期台账触发）、以及同日重跑让 due 塌到个位数——两次都会让覆盖校验对"几乎没采集"的日报放行。根因归类：**隐式假设**（默认降级路径是安全的）。凡是"基准变窄"的降级都要 fail-closed。
4. **目录级 `git add` 把上个会话的遗留改动搭车带入分支**，终审发现后已用 revert commit 剥离并把改动放回工作区。后续提交应逐文件 add。
5. **验收口径的基线数字是拍脑袋的**：intake 写"基线约 110"，实测 14 天均值 148.3。口径应该在有台账数据后再定，而不是在 intake 阶段凭 6 天抽样估。
6. **最大的教训是范围判断，不是实现质量**：cadence 调度写完、测完、过了五轮 review 才发现它的收益上限（21%）在 intake 定豁免名单那一刻就注定了。**「这个机制能覆盖多少比例的场景」应该在 intake 阶段量化，而不是在 design 里写一句定性风险**（当时写的是「豁免过宽则总节省达不到 30%」，没算数）。五轮 review 的 12 条 findings 全部落在这部分，而真正有价值的 surface_kind 零 findings——低性价比的机制不仅自己不划算，还会挤占高价值改动的评审预算。最终整体撤销。
